...
@app.get("/recommend/{user_id}/top-url", response_class=PlainTextResponse)
async def get_top_recommended_url(user_id: str):
    """API endpoint to get the top recommended URL for a specified UserId."""
    try:
        logger.info("Received request for top recommended URL for UserId: %s", user_id)
        ...
        # Extract the top recommended URL and append a newline
        top_url = result["ad_url"]
        logger.info("Returning top recommended URL for UserId: %s: %s", user_id, top_url)
        return top_url + "\n"  # Append newline for better formatting in terminal
